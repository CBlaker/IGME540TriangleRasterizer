...
debug Game::initializeDebugInfo() {
	...
	debugInfo.DisplayGuiMenu = false;
	return debugInfo;
}
...
Game::Game()
{
	...
	//Choose ImGui window style/color
...
//Helper function to update ImGui with yummy new data
void Game::UpdateGuiData(float deltaTime, debug guiInfo) {
...
//Actual UI setup build
void Game::BuildUI(debug& guiInfo, int framesElapsed) {
	...
	if (ImGui::TreeNode("Debug Info")) {
		...
		ImGui::Text("Resolution: %dx%d", Window::Width(), Window::Height());
	...
	if (ImGui::TreeNode("Debug Settings")) {
		ImGui::ColorEdit3("Background Color", &guiInfo.bgColor.x);

		 if (ImGui::Button("Toggle ImGui Demo Window")) {
			guiInfo.DisplayGuiMenu = !guiInfo.DisplayGuiMenu; }
...
// --------------------------------------------------------
void Game::Update(float deltaTime, float totalTime, debug& debugInfo) {

	//Get ImGui Data
	Game::UpdateGuiData(deltaTime, debugInfo);

	//Build UI
	Game::BuildUI(debugInfo, int(totalTime));
...
// --------------------------------------------------------
void Game::Draw(float deltaTime, float totalTime, debug& debugInfo)
{
	// Frame START
	...
	// - At the beginning of Game::Draw() before drawing *anything*
	{
		...
		const float color[4] = { debugInfo.bgColor.x, debugInfo.bgColor.y, debugInfo.bgColor.z, 1.0f };
